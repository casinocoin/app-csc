...
void readAmount(parseContext_t *context, field_t *field) {
    ...
    if ((firstByte >> 7u) == 0) {
        ...
        if (hasNonStandardCurrency(field)) {
            ...
            currency->id = CSC_CURRENCY_CURRENCY;
...
void handlePathStep(parseContext_t *context, field_t *field, uint8_t stepType) {
    ...
    switch (stepType) {
        case 0x01:
            ...
            field->id = CSC_ACCOUNT_ACCOUNT;
            break;
        case 0x20:
            field->dataType = STI_ACCOUNT;
            field->id = CSC_ACCOUNT_ISSUER;
            break;
        case 0x30:
        ...
        case 0x10:
            ...
            field->id = CSC_CURRENCY_CURRENCY;
            break;
        default:
...
void postProcessField(parseContext_t *context, field_t *field) {
    switch (field->dataType) {
        ...
        case STI_VL:
            ...
            if (field->id == CSC_VL_SIGNING_PUB_KEY && field->length == 0) {
...
void postProcessTransaction(parseContext_t *context) {
    ...
    if (context->transactionType == TRANSACTION_SET_REGULAR_KEY && !hasField(context, STI_ACCOUNT, 8)) {
        ...
        field->dataType = STI_ACCOUNT;
        field->id = CSC_ACCOUNT_REGULAR_KEY;
