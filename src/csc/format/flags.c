...
bool isFlag(field_t* field) {
    return field->dataType == STI_UINT32 &&
            (
                field->id == CSC_UINT32_FLAGS ||
                field->id == CSC_UINT32_SET_FLAG ||
                field->id == CSC_UINT32_CLEAR_FLAG
            );
}
...
void formatAccountSetTransactionFlags(uint32_t value, char *dst) {
    ...
    #define TF_DISALLOW_CSC        0x00100000u
    #define TF_ALLOW_CSC           0x00200000u
    ...
    if (HAS_FLAG(value, TF_DISALLOW_CSC)) {
        dst = appendItem(dst, "Disallow CSC");
    }

    if (HAS_FLAG(value, TF_ALLOW_CSC)) {
        dst = appendItem(dst, "Allow CSC");
    }
}
...
void formatAccountSetFieldFlags(uint32_t value, char *dst) {
    ...
    switch (value) {
        ...
        case ASF_DISABLE_MASTER:
            ...
            break;
        case ASF_DISALLOW_CSC:
            strcpy(dst, "Disallow CSC");
            break;
